from estimator.nms import pose_nms
...
class PoseEstimator(object):
    def __init__(self):
        ...
        if config.pose_backbone == "seresnet101":
            from estimator.pose_model.seresnet.FastPose import InferenNet_fast as createModel
            self.pose_model = createModel(4 * 1 + 1, pose_dataset, cfg=config.pose_cfg)
        elif config.pose_backbone == "mobilenet":
            from estimator.pose_model.mobilenet.MobilePose import createModel
