...
def vis_frame_fast(frame, im_res, format='coco'):
    ...
    if format == 'coco':
        l_pair = coco_l_pair
        p_color = coco_p_color
        line_color = coco_line_color
    elif format == 'mpii':
        l_pair = mpii_l_pair
        p_color = mpii_p_color
        line_color = mpii_line_color
    else:
...
def vis_frame_black(frame, im_res, format='coco'):
    '''
    frame: frame image
    im_res: im_res of predictions
    format: coco or mpii
    ...
    black = cv2.resize(black,(int(width/2), int(height/2)))
    # remove the background
    ...
    for human in im_res['result']:
        ...
        for n in range(kp_scores.shape[0]):
            ...
            if n == 15:
                Lankle.append(cor_x/2)
                Lankle.append(cor_y/2)
            if n == 16:
                Rankle.append(cor_x/2)
                Rankle.append(cor_y/2)

            if np.size(Lankle) >=2 and np.size(Rankle) >=2:
                distance = np.square(Lankle[0] - Rankle[0]) + np.square(Lankle[1] - Rankle[1])
                distance = int(np.sqrt(distance))

            part_line[n] = (int(cor_x/2), int(cor_y/2))

            bg = black.copy()#remove the background
...
def vis_frame(frame, im_res, format='coco'):
    ...
    for human in im_res['result']:
        ...
        for n in range(kp_scores.shape[0]):
            ...
            #bg = black.copy()  # remove the background
            # if distance > 0:
            #     cv2.putText(bg,str(distance),(100,100),cv2.FONT_HERSHEY_SIMPLEX, 2 , (255,255,255), 2)
            # if count > 0:
            #     cv2.putText(bg, str(count), (100, 150), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 2)
